...
void Go_Down_Red_Stair(int &number)
{
	...
	static bool GO_DOWN_CHECK = false;
    ...
    if(GO_DOWN_CHECK || max_area <= 2600 || temp_i == 0)
    {
		GO_DOWN_CHECK = true;
		Motion_Command(SoundPlay);
        Motion_Command(4);
...
void Yellow_Trap(int &number)
{
	...
	if (!Look_Down_Check) {
		...
		for (i = 0; i<area.size(); i++)
		{
			if (area[i].first.first >= 1800 && area[i].first.first>max_area)
			{
				max_area = area[i].first.first;
				temp_i = i + 1;
			}
...
void Up_Yellow_Stair(int &number)
{
	//
	//Motion_Command(UP_RED_STAIR);
	U16 *input = (U16*)malloc(2 * 180 * 120);
	...
	Motion_Command(11);//노란 함정 올라가기로 수정 요망
	number++;
	draw_fpga_video_data_full(input);
	flip();
	free(input);
